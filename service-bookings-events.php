...
class Service_Bookings_Events {
    ...
    public function bookings_page() {
        ...
        ?><div class="wrap"><h1><?php echo esc_html__('All Bookings', 'service-bookings-events'); ?></h1><table class="wp-list-table widefat fixed striped"><thead><tr><th><?php echo esc_html__('ID', 'service-bookings-events'); ?></th><th><?php echo esc_html__('Customer', 'service-bookings-events'); ?></th><th><?php echo esc_html__('Date & Time', 'service-bookings-events'); ?></th><th><?php echo esc_html__('Status', 'service-bookings-events'); ?></th><th><?php echo esc_html__('Payment', 'service-bookings-events'); ?></th></tr></thead><tbody><?php if (empty($bookings)): ?><tr><td colspan="5"><?php echo esc_html__('No bookings found.', 'service-bookings-events'); ?></td></tr><?php else: ?><?php foreach ($bookings as $booking): ?><tr><td>#<?php echo esc_html($booking->id); ?></td><td><?php echo esc_html($booking->customer_name); ?><br><small><?php echo esc_html($booking->customer_email); ?></small></td><td><?php echo esc_html(date_i18n(get_option('date_format'), strtotime($booking->booking_date)) . ' ' . date_i18n(get_option('time_format'), strtotime($booking->booking_time))); ?></td><td><span class="sbe-status sbe-status-<?php echo esc_attr($booking->status); ?>"><?php echo esc_html(ucfirst($booking->status)); ?></span></td><td><?php if ($booking->payment_status === 'paid'): ?><span style="color: #46b450;"><?php echo esc_html__('Paid'); ?></span><?php elseif ($booking->payment_status): ?><?php echo esc_html(ucfirst($booking->payment_status)); ?><?php else: ?>-<?php endif; ?></td></tr><?php endforeach; ?><?php endif; ?></tbody></table></div><?php
    }
